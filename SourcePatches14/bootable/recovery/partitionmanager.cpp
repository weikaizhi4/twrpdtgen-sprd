...
static inline std::string KM_Ver_From_Manifest(std::string ver) {
	...
	if (ver.empty()) {
		std::string keymint_ver;
		if (TWFunc::Get_Service_From_Manifest("/vendor", "android.hardware.security.keymint", keymint_ver))
			ver = "keymint-" + keymint_ver;
	}
...
clear:
	...
	#ifndef TW_EMBEDDED_VENDOR_LIBS
		if (ven) ven->Mount(Display_Error);
	#endif
		if (odm) odm->Mount(Display_Error);

	#ifdef TW_EMBEDDED_VENDOR_LIBS
		Process_ResetProps(nullptr, odm);
	#else
...
void TWPartitionManager::Decrypt_Data() {
	...
	if (Decrypt_Data && Decrypt_Data->Is_Encrypted && !Decrypt_Data->Is_Decrypted) {
...
#ifdef TW_INCLUDE_FBE_METADATA_DECRYPT
#ifdef USE_FSCRYPT
			std::string metadata_fstab;
			if (TWFunc::Path_Exists(additional_fstab)) {
				metadata_fstab = additional_fstab;
			}
#ifdef TW_METADATA_FSTAB_PATH
			else if (TWFunc::Path_Exists(TW_METADATA_FSTAB_PATH)) {
				metadata_fstab = TW_METADATA_FSTAB_PATH;
			}
#endif
			LOGINFO("Metadata decrypt fstab: '%s'\n", metadata_fstab.c_str());
			int metadata_decrypt_attempts = 1;
#ifdef TW_METADATA_FSTAB_PATH
			metadata_decrypt_attempts = 25;
#endif
			bool metadata_decrypted = false;
			for (int attempt = 1; attempt <= metadata_decrypt_attempts; ++attempt) {
				...
				if (attempt < metadata_decrypt_attempts) {
					LOGINFO("Metadata decrypt attempt %d/%d failed; retrying after Trusty storage startup\n", attempt, metadata_decrypt_attempts);
					sleep(1);
				}
			}
		...
		if (Decrypt_Data->Is_FBE) {
			...
			LOGERR("FBE setup failed; legacy FDE is not supported by this Android 15 recovery build.\n");
		}
	}
	if (Decrypt_Data && (!Decrypt_Data->Is_Encrypted || Decrypt_Data->Is_Decrypted)) {
		Decrypt_Adopted();
...
int TWPartitionManager::Decrypt_Device(string Password, int user_id) {
	...
	// Android 15 no longer exposes the legacy cryptfs FDE API. Keep this
	// fallback explicit so FBE failures are reported instead of calling symbols
	// that cannot be linked into recovery.
	for (iter = Partitions.begin(); iter != Partitions.end(); iter++) {
		if ((*iter)->Mount_To_Decrypt) {
			(*iter)->UnMount(false);
		}
	...
	gui_err("fde_unsupported=Legacy FDE is not supported by this recovery build.");
	return -1;
#else

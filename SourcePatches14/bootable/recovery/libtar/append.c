...
tar_append_file(TAR *t, const char *realname, const char *savename)
{
	...
	if (TH_ISDIR(t) && t->options & TAR_STORE_FSCRYPT_POL)
	{
		...
		t->th_buf.fep = malloc(sizeof(*t->th_buf.fep));
		if (!t->th_buf.fep) {
		...
		if (fscrypt_policy_get_struct(realname, t->th_buf.fep)) {
			...
			size = FSCRYPT_KEY_IDENTIFIER_SIZE;
			hex_size = FSCRYPT_KEY_IDENTIFIER_HEX_SIZE;
			descriptor = t->th_buf.fep->master_key_identifier;
			const char *user_ce = USER_CE_FSCRYPT_POLICY;
			const char *user_de = USER_DE_FSCRYPT_POLICY;
			const char *system_de = SYSTEM_DE_FSCRYPT_POLICY;
#ifdef DEBUG

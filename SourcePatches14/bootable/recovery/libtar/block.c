	...
	while(TH_ISEXTHEADER(t) || TH_ISPOLHEADER(t))
	{
		...
		else
		{
			...
			if (start && start+FSCRYPT_TAG_LEN < buf+len) {
				...
				if (*start == '0' && start + 1 + sizeof(*t->th_buf.fep) <= buf + len) {
					start++;
					memcpy(t->th_buf.fep, start, sizeof(*t->th_buf.fep));
					if (t->th_buf.fep->version != FSCRYPT_POLICY_V2) {
						LOG("unsupported fscrypt policy version: %u\n", t->th_buf.fep->version);
						free(t->th_buf.fep);
						t->th_buf.fep = NULL;
					}
#ifdef DEBUG
					LOG("    th_read(): FSCrypt v2 policy detected\n");
#endif
				}
				else {

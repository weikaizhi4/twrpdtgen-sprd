	...
	if (strcmp(crypto_blkdev, "error") != 0) {
		...
		if (Is_Present) {
			if (Key_Directory.empty()) {
				LOGERR("Unable to mount /data without an FBE keydirectory; legacy FDE is unsupported.\n");
				gui_err("mount_data_fde=Could not mount /data: legacy FDE is not supported.");
			} else {
				Is_Encrypted = true;
				Is_Decrypted = false;
				if (datamedia)

...
#ifdef TW_INCLUDE_FBE
	if (Mount_Point == "/data" && !(flags & MS_RDONLY) &&
			TWFunc::Path_Exists("/data/unencrypted/key/version")) {
		static bool reloading_fbe_keys = false;

		if (!reloading_fbe_keys) {
			reloading_fbe_keys = true;
			LOGINFO("Reloading FBE keys after mounting /data\n");
			if (!android::keystore::Decrypt_DE())
				LOGINFO("Unable to reload FBE keys after mounting /data\n");
			reloading_fbe_keys = false;
		}
	}
#endif
	return true;

...
static bool String_Contains_Case_Insensitive(const string& haystack, const string& needle) {
	string lower_haystack;
	string lower_needle;

	for (char c : haystack)
		lower_haystack += static_cast<char>(tolower(static_cast<unsigned char>(c)));
	for (char c : needle)
		lower_needle += static_cast<char>(tolower(static_cast<unsigned char>(c)));

	return lower_haystack.find(lower_needle) != string::npos;
}

static unsigned long Normalize_Temperature(const string& raw) {
	unsigned long temp = strtoul(raw.c_str(), NULL, 0);
...
static bool Read_Temperature_File(const string& path, unsigned long& temp) {
	string raw;

	if (TWFunc::read_file(path, raw) != 0)
		return false;

	temp = Normalize_Temperature(raw);
	return temp > 0 && temp < 150;
}

static bool Is_CPU_Thermal_Zone(const string& type) {
	return String_Contains_Case_Insensitive(type, "cpu") ||
		String_Contains_Case_Insensitive(type, "apcpu") ||
		String_Contains_Case_Insensitive(type, "big") ||
		String_Contains_Case_Insensitive(type, "mid") ||
		String_Contains_Case_Insensitive(type, "lit") ||
		String_Contains_Case_Insensitive(type, "cluster");
}

static bool Find_CPU_Temperature_File(string& path, unsigned long& temp) {
	...
	while ((entry = readdir(dir)) != NULL) {
		string zone = entry->d_name;
		if (zone.find("thermal_zone") != 0)
			continue;

		string zone_path = "/sys/class/thermal/" + zone;
		string type;
		if (TWFunc::read_file(zone_path + "/type", type) != 0 || !Is_CPU_Thermal_Zone(type))
			continue;

		if (Read_Temperature_File(zone_path + "/temp", temp)) {
			path = zone_path + "/temp";
			closedir(dir);
			return true;
		}
...
static bool Find_Fallback_Temperature_File(string& path, unsigned long& temp) {
	const char* candidates[] = {
		"/sys/devices/platform/charger-manager/power_supply/battery/temp",
		"/sys/devices/platform/soc/soc:aon/64400000.spi/spi_master/spi4/spi4.0/sc27xx-fgu/power_supply/sc27xx-fgu/temp",
		"/sys/devices/platform/charger-manager/power_supply/battery/temp_ambient",
		NULL
	};

	for (const char** candidate = candidates; *candidate; candidate++) {
		if (Read_Temperature_File(*candidate, temp)) {
			path = *candidate;
			return true;
		}
	}
...
static bool Find_Temperature_File(string& path, unsigned long& temp) {
	if (Find_CPU_Temperature_File(path, temp))
		return true;

	return Find_Fallback_Temperature_File(path, temp);
}
...
void DataManager::SetDefaultValues()
{
	...
	if (TWFunc::Path_Exists(cpu_temp_file)) {
		...
		unsigned long detected_temp = 0;
		string detected_path;
		if (Find_Temperature_File(detected_path, detected_temp)) {
			LOGINFO("Using detected temperature file '%s'.\n", detected_path.c_str());
			mConst.SetValue("tw_no_cpu_temp", "0");
		} else {
...
int DataManager::GetMagicValue(const string& varName, string& value)
{
	...
	else if (varName == "tw_cpu_temp")
	{
		...
		string cpu_temp_file;
		static string detected_cpu_temp_file;
		...
		{
			if (!detected_cpu_temp_file.empty()) {
				if (!Read_Temperature_File(detected_cpu_temp_file, convert_temp))
					detected_cpu_temp_file.clear();
			}
			...
			if (detected_cpu_temp_file.empty()) {
				...
				if (TWFunc::read_file(cpu_temp_file, results) != 0)
					return -1;
#endif
				convert_temp = Normalize_Temperature(results);
				if (convert_temp <= 0 || convert_temp >= 150) {
					unsigned long detected_temp = 0;
					string detected_path;
					if (Find_Temperature_File(detected_path, detected_temp)) {
						LOGINFO("Using detected temperature file '%s'.\n", detected_path.c_str());
						detected_cpu_temp_file = detected_path;
						convert_temp = detected_temp;
					}
				}
			}

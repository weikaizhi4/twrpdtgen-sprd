...
bool installKey(const std::string& mountpoint, const EncryptionOptions& options,
    ...
    if (!installProvisioningKey(key, ref, arg->key_spec)) {
        if (android::base::GetProperty("ro.bootmode", "") == "recovery") {
            LOG(WARNING) << "Continuing without fscrypt provisioning key in recovery";
        } else {
            return false;
        }
    }
